...
def create_user(**params):
    """Create a user with the given parameters. and return a new user."""
...
class PublicUserApiTests(TestCase):
    ...
    def test_user_with_email_exists(self):
        """Test that a user can be created with email is exist."""
        payload = {
            'email': 'test@example.com',
            'password': 'testpass123',
            'name': 'Test Name',
    ...
    def test_create_token_bad_credentials(self):
        ...
        payload = {'email': '', 'password': 'bad-pass'}
        res = self.client.post(TOKEN_URL, payload)
        self.assertNotIn('token', res.data)
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_create_token_blank_password(self):
        """Test that token is not created for blank password. return  an error"""
